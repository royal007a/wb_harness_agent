...
- 当前阶段：`baidu_netdisk_oauth_verified`
- 是否开始实现：是，用户于 2026-09-12 授权本地初版前后端和部署
- 当前任务：无；百度网盘 OAuth 连接器数据面仍关闭
- 最近完成：`HA-0011` 百度网盘真实 OAuth 连通及无数据面 refresh 验证；证据位于 `harness/evidence/HA-0011/`
...
- 当前阻塞：`HA-0008` 仍等待本项目批准的模型端点、模型 ID、凭证引用和预算；TD-020 未关闭，百度网盘文件数据面与内容解析仍不开放。
- 当前发布：`0.1.0-local`，http://127.0.0.1:8765
- 最后检查点：114 项完整测试通过；百度网盘 OAuth 的真实一次 refresh 仅访问官方 token endpoint，且无文件 API 调用/无密 Evidence。连接入口 `/connectors/baidu-netdisk`。OAuth 数据面关闭，Claude SDK 0.2.152 仅做离线配置验证，真实模型路由未开启。
