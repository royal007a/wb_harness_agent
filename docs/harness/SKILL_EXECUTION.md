...
`skills/baidu-netdisk-download/` 是受限的本机交接 Skill：只从当前用户明确消息或引用消息读取 `https://pan.baidu.com/s/...`，经 stdin 交给已安装的官方 macOS 客户端，绝不回显或保存 URL、`pwd` 或凭证。用户自行在客户端完成登录、提取码确认和下载；完成后脚本只认领用户明确指定的 `~/Downloads` 普通文件路径，不读取其内容。
